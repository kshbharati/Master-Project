...
def read_root(db:Session=Depends(get_db)):
    return crud.get_assigned_courses_for_student(db=db, studentId=1)
...
@app.post("/course/assign", response_model=schemas.AssignedCourse)
def assign_course(asscourse: schemas.AssignedCourseCreate, db: Session = Depends(get_db)):
